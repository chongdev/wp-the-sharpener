...
define( 'GRID_ITEMS' , '' );
define( 'MODAL_SURVEY_VERSION' , '2.0.1.2' );
define( 'MSDIRS' , '/' );
	...
	class modal_survey {
		...
		global $wpdb;
			...
			$survey->options = array(
				...
				/*"icontact" => $options[ 57 ],
				"icontact_appid" => $options[ 58 ],
				"icontact_apiusername" => $options[ 59 ],
				"icontact_apipassword" => $options[ 60 ],
				"icontact_listid" => $options[ 61 ],
				"infusionsoft" => $options[ 62 ],
				"infusionsoft_apikey" => $options[ 63 ],
				"infusionsoft_campaignid" => $options[ 64 ],
				"infusionsoft_groupid" => $options[ 65 ],
				"interspire" => $options[ 66 ],
				"interspire_username" => $options[ 67 ],
				"interspire_usertoken" => $options[ 68 ],
				"interspire_listid" => $options[ 69 ],*/
				"madmimi" => $options[ 70 ],
				...
				/*"mailerlite" => $options[ 77 ],
				"mailerlite_apikey" => $options[ 78 ],
				"mailerlite_listid" => $options[ 79 ],
				"mailigen" => $options[ 80 ],
				"mailigen_apikey" => $options[ 81 ],
				"mailigen_listid" => $options[ 82 ],
				"mailigen_doubleoptin" => $options[ 83 ],
				"mailjet" => $options[ 84 ],
				"mailjet_apikey" => $options[ 85 ],
				"mailjet_secretkey" => $options[ 86 ],
				"mailjet_listid" => $options[ 87 ],*/
				"mailpoet" => $options[ 88 ],
				"mailpoet_listid" => $options[ 89 ],
				/*"emma" => $options[ 90 ],
				"emma_accountid" => $options[ 91 ],
				"emma_publickey" => $options[ 92 ],
				"emma_privatekey" => $options[ 93 ],*/
				"mymail" => $options[ 94 ],
				"mymail_listid" => $options[ 95 ],
				/*"ontraport" => $options[ 96 ],
				"ontraport_appid" => $options[ 97 ],
				"ontraport_key" => $options[ 98 ],
				"ontraport_tagid" => $options[ 99 ],
				"ontraport_sequenceid" => $options[ 100 ],
				"pinpointe" => $options[ 101 ],
				"pinpointe_username" => $options[ 102 ],
				"pinpointe_usertoken" => $options[ 103 ],
				"pinpointe_listid" => $options[ 104 ],
				"sendinblue" => $options[ 105 ],
				"sendinblue_accesskey" => $options[ 106 ],
				"sendinblue_listid" => $options[ 107 ],
				"sendreach" => $options[ 108 ],
				"sendreach_key" => $options[ 109 ],
				"sendreach_secret" => $options[ 110 ],
				"sendreach_userid" => $options[ 111 ],
				"sendreach_listid" => $options[ 112 ],
				"sendy" => $options[ 113 ],
				"sendy_installationurl" => $options[ 114 ],
				"sendy_apikey" => $options[ 115 ],
				"sendy_listid" => $options[ 116 ],*/
			...
			$survey->questions = $wpdb->get_results( "SELECT * FROM " . $wpdb->base_prefix . "modal_survey_questions WHERE `survey_id`='" . $sid . "' ORDER BY id ASC" );
			...
			$survey->questions[ $key ]->options = array(
				...
				$survey->questions[ $key ]->answers = $wpdb->get_results( "SELECT * FROM " . $wpdb->base_prefix . "modal_survey_answers WHERE `survey_id`='" . $sid . "' AND `question_id`='" . $qs->id . "' ORDER BY autoid ASC" );
		...
		**/
		...
		public function survey_conditions_shortcodes( $atts, $content = null ) {
			...
			if ( $args[ 'session' ] == "last" ) {
				...
				$last_session = "SELECT samesession FROM " . $wpdb->base_prefix . "modal_survey_participants_details WHERE sid = '" . $args[ 'id' ] . "' AND uid = '" . $mscuid . "' ORDER BY time DESC";
			...
			$fullrecords = modal_survey::survey_answers_shortcodes( 
				array ( 'id' => $args[ 'id' ], 'data' => 'full-records', 'style' => 'plain', 'uid' => 'true', 'pure' => 'true', 'session' => 'last'  )
			);
			if ( $args[ 'advanced' ] != '' ) {
		...
		public function survey_useraction( $atts ) {
			...
			if ( empty( $atts[ 'id' ] ) )	{
				return __( 'Survey ID parameter is empty, please specify.', MODAL_SURVEY_TEXT_DOMAIN );
			}
			if ( isset( $_COOKIE[ 'ms-uid' ] ) ) {
				...
				if ( isset( $_REQUEST[ 'modalsurvey_useraction_button' ] ) ) {
					...
					if ( $_REQUEST[ 'modalsurvey_useraction_button' ] == "delete" ) {
						...
						if ( $result ) {
							echo '<div class="updated"><p>'.__( 'Successfully Deleted!', MODAL_SURVEY_TEXT_DOMAIN ).'</p></div>';
						}
						else {
		...
		public function survey_calculation_shortcodes( $atts ) {
			...
			if ( empty( $atts[ 'math' ] ) )	{
				return __( 'Math parameter is empty, please specify.', MODAL_SURVEY_TEXT_DOMAIN );
			}
				$ssuid = "";
			...
			foreach( $process_math as $key => $pm ) {
				...
				elseif ( ! is_numeric( floatval( $score ) ) ) {
					return __( $score . ' is not numeric, it is not possible to create calculations with non-numeric values.', MODAL_SURVEY_TEXT_DOMAIN );
				}
			...
			if ( $decs > 0 ) {
				return number_format( $calc_results, 2 );
			}
			if ( is_numeric( $calc_results ) ) {
				return number_format( $calc_results, 0 );
			}
			return __( $calc_results . ' result is not numeric, it is possible to create calculations with numeric values only.', MODAL_SURVEY_TEXT_DOMAIN );
		}
		...
		public function survey_open_answers_shortcodes( $atts ) {
			...
			if ( ! empty( $list_openanswers ) ) {
				...
				$result .= "<div class='each-open-answer ms-ota-" . $key . "'>" . __( 'Data doesn\'t exists', MODAL_SURVEY_TEXT_DOMAIN ) . "</div>";				
			}
			$result .= "</div>";
			return $result;
		}
		...
		public function survey_records_shortcodes( $atts ) {
			...
			if ( $atts[ 'session' ] == "last" ) {
				if ( $atts[ 'uid' ] == "true" ) {
					if ( isset( $_COOKIE[ 'ms-uid' ] ) ) {
						//$ssuid = $wpdb->get_var( $wpdb->prepare( "SELECT id FROM " . $wpdb->base_prefix . "modal_survey_participants WHERE id = %s ", $_COOKIE[ 'ms-uid' ] ) );
				...
				$last_session = "SELECT samesession FROM " . $wpdb->base_prefix . "modal_survey_participants_details WHERE sid = '" . $atts[ 'id' ] . "' AND uid = '" . $current_autoid . "' ORDER BY time DESC";
			...
			$sql_u = "SELECT autoid as id, username, email, name, custom FROM " . $wpdb->base_prefix . "modal_survey_participants WHERE id = '" . $ssuid . "'";
			...
			if ( $args[ 'style' ] == 'plain' ) {
				...
				if ( $args[ 'data' ] != 'full-records' ) {
					...
					if ( $args[ 'printable' ] == "true" ) {
						$result .= '<input type="button" class="ms-print-results button button-secondary" data-id="survey-results-' . $args[ 'id' ] . '-' . $unique_key . '" value="' . __( 'PRINT', MODAL_SURVEY_TEXT_DOMAIN ) . '" />';
					}
				}
						$finaltimer = 0;
						$finalscore = 0;
						...
						//display individual records end
							...
							$result .= "<div class='question-onerow'><div class='ms-question-row'><div class='ms-question-text'>" . $args[ 'title' ] . preg_replace( '/\[.*\]/', '', $ars->question ) . $args[ 'title_c' ] . "</div><div class='ms-question-block1'></div><div class='ms-question-block2'>" . ( isset( $timer[ $key1 + 1 ] ) && $finaltimer > 0  ? ( __( 'Time Required', MODAL_SURVEY_TEXT_DOMAIN ) . ": ". $timer[ $key1 + 1 ] . __( 'sec', MODAL_SURVEY_TEXT_DOMAIN ) ) : '' ) . "</div></div>";
								...
								foreach( $a_sql as $key2=>$as ) {
									...
									$checkcorrect = '';										
									if ( $aoptions[ 5 ] == '1' ) {
										$checkcorrect = '<div class="correct-mark modal_survey_tooltip" title="' . __( 'Correct Answer', MODAL_SURVEY_TEXT_DOMAIN ) . '"><img src="' . plugins_url( '/templates/assets/img/correct-icon.png' , __FILE__ ) . '"></div>';
									}
									$result .= "<div class='ms-answer-row" . $selected . "'>" . $checkcorrect . "<div class='ms-answer-text'>" . ( preg_replace( '/\[.*\]/', '', $as->answer ) ? ( preg_replace( '/\[.*\]/', '', $as->answer ) ) : __( 'Not Specified', MODAL_SURVEY_TEXT_DOMAIN ) ) . "</div><div class='ms-answer-count modal_survey_tooltip' title='" . __( 'Global Votes', MODAL_SURVEY_TEXT_DOMAIN ) . "'>" . $as->count . "</div><div class='ms-answer-percentage modal_survey_tooltip' title='" . __( 'Global Percentage', MODAL_SURVEY_TEXT_DOMAIN ) . "'>" . ( $allcount > 0 ? ( round( ( $as->count / $allcount ) * 100, 2 ) ) : '0' ) . "%" . "</div>" . $score_output . "</div>";
								}
								$result .= "</div>";
		...
		public function survey_shortcodes( $atts ) {
			...
			extract( shortcode_atts( array(
					...
					'align' => 'left',
					'textalign' => 'center',
					'message' => __( 'You already filled out this survey!', MODAL_SURVEY_TEXT_DOMAIN ),
					'filtered' => 'false',
					'social' => '',
					'init' => '',
					'visible' => 'true',
			...
			if ( ( ! isset( $atts[ 'message' ] ) || $atts[ 'message' ] == '' ) && ( $atts[ 'style' ] == 'click' || $atts[ 'style' ] == 'flat' ) ) {
				$atts[ 'message' ] = __( 'You already filled out this survey!', MODAL_SURVEY_TEXT_DOMAIN );
			}
			if ( ! isset( $atts[ 'id' ] ) ) {
				$atts[ 'id' ] = '';
			}
		...
		function extend_the_content( $content ) {
			...
			if ( $this->auto_embed == 'false' && ! empty( $content ) ) {
				$this->auto_embed = 'true';
				$sql = "SELECT id, options FROM " . $wpdb->base_prefix . "modal_survey_surveys mss WHERE global = 1 AND (`expiry_time`>'".current_time( 'mysql' )."' OR `expiry_time`='0000-00-00 00:00:00') AND (`start_time`<'" . current_time( 'mysql' ) . "' OR `start_time`='0000-00-00 00:00:00')";
		...
		global $current_user;
			...
			$so = json_decode( $array['survey_options'] );
				$so->languages = array(
					...
					"confirm_button" => __( 'CONFIRM', MODAL_SURVEY_TEXT_DOMAIN ),
					"confirm_title" => __( 'OVERVIEW OF ALL GIVEN ANSWERS', MODAL_SURVEY_TEXT_DOMAIN ),
					"confirm_question" => __( 'QUESTION', MODAL_SURVEY_TEXT_DOMAIN ),
					"confirm_answer" => __( 'ANSWER', MODAL_SURVEY_TEXT_DOMAIN ),
					"confirm_edit" => __( 'EDIT', MODAL_SURVEY_TEXT_DOMAIN ),
					...
					"share" => __( 'Share', MODAL_SURVEY_TEXT_DOMAIN ),
					"tweet" => __( 'Tweet', MODAL_SURVEY_TEXT_DOMAIN ),
					"plusone" => __( '+1', MODAL_SURVEY_TEXT_DOMAIN ),
					"pinit" => __( 'Pin it', MODAL_SURVEY_TEXT_DOMAIN )
					);
