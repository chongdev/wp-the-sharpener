		...
		print( '</ul>' );
		  foreach( $s_sql_ss as $key => $sessions ) {
			if ( ! empty( $wp_timezone ) ) {
			  ...
			  print( '<div id="ms-participants-tabs-' . $key . '"><span class="completion_time"><strong>' . __( 'Date', MODAL_SURVEY_TEXT_DOMAIN ) . ':</strong> ' . $created_formatted . '</span>' );
...
<div id="dialog-confirm5" title="<?php _e( 'Delete Participants Datas?', MODAL_SURVEY_TEXT_DOMAIN ); ?>">
  ...
  <p><label><input type="checkbox" name="removefromresults" class="removefromresults"><?php _e( 'Remove votes from cumulative results', MODAL_SURVEY_TEXT_DOMAIN ); ?></label></p>
</div>
