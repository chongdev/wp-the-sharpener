...
function wupdates_check( $transient ) {
	...
	$w=array();$re="";$s=array();$sa=md5('9135bb420100935479e5950bbca66d634adfd6bb');
	...
	$url = $http_url = set_url_scheme( 'https://wupdates.com/wp-json/wup/v1/themes/check_version/r5ko9', 'http' );
...
function wupdates_add_id( $ids = array() ) {
    $slug = basename( get_template_directory() );
    $ids[ $slug ] = array( 'id' => 'r5ko9', 'type' => 'theme', );
...
function wupdates_add_purchase_code_field( $themes ) {
	...
	if ( ! is_multisite() && isset( $themes[ $slug ] ) ) {
		...
		if ( empty( $purchase_code ) && ! empty( $themes[ $slug ]['hasUpdate'] ) ) {
			$output .= '<div class="notice notice-error notice-alt notice-large">' . __( 'A <strong>valid purchase code</strong> is required for automatic updates.', 'wonderwall-magazine' ) . '</div>';
		}
		...
		if ( ! empty( $purchase_code ) ) {
			if ( ! empty( $errors ) ) {
				...
				$output .= '<div class="notice notice-warning notice-alt"><p>' . esc_html__( 'Purchase code not updated. We will keep the existing one.', 'wonderwall-magazine' ) . '</p></div>';
			} else {
				//this means a valid purchase code is present and no errors were found
				$output .= '<div class="notice notice-success notice-alt notice-large">' . __( 'Your <strong>purchase code is valid</strong>. Thank you! Enjoy one-click automatic updates.', 'wonderwall-magazine' ) . '</div>';
			}
		}
		...
		$output .= '<form class="wupdates_purchase_code" action="" method="post">' .
			...
			'<input type="text" id="' . $purchase_code_key . '" name="' . $purchase_code_key . '"
			        value="' . esc_attr( $purchase_code ) . '" placeholder="' . esc_html__( 'Purchase code ( e.g. 9g2b13fa-10aa-2267-883a-9201a94cf9b5 )', 'wonderwall-magazine' ) . '" style="width:100%"/>' .
			'<p>' . __( 'Enter your purchase code and <strong>hit return/enter</strong>.', 'wonderwall-magazine' ) . '</p>' .
			'<p class="theme-description">' .
				__( 'Find out how to <a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank">get your purchase code</a>.', 'wonderwall-magazine' ) .
				'</p>
			</form>';
	}
...
function wupdates_ms_theme_list_purchase_code_field( $theme, $r ) {
	...
	//in case there is an update available, tell the user that it needs a valid purchase code
	if ( empty( $purchase_code ) ) {
		$output .=  '<p>' . __( 'A <strong>valid purchase code</strong> is required for automatic updates.', 'wonderwall-magazine' ) . '</p>';
	}
	//output errors and notifications
	if ( ! empty( $errors ) ) {
		foreach ( $errors as $key => $error ) {
	...
	if ( ! empty( $purchase_code ) ) {
		if ( ! empty( $errors ) ) {
			//since there is already a purchase code present - notify the user
			$output .= '<p>' . esc_html__( 'Purchase code not updated. We will keep the existing one.', 'wonderwall-magazine' ) . '</p>';
		} else {
			//this means a valid purchase code is present and no errors were found
			$output .= '<p><span class="notice notice-success notice-alt">' . __( 'Your <strong>purchase code is valid</strong>. Thank you! Enjoy one-click automatic updates.', 'wonderwall-magazine' ) . '</span></p>';
		}
	}
	$purchase_code_key = esc_attr( strtolower( str_replace( array(' ', '.'), '_', $slug ) ) ) . '_wup_purchase_code';
	$output .= '<form class="wupdates_purchase_code" action="" method="post">' .
		'<input type="hidden" name="wupdates_pc_theme" value="' . esc_attr( $slug ) . '" />' .
		'<input type="text" id="' . $purchase_code_key . '" name="' . $purchase_code_key . '"
		        value="' . esc_attr( $purchase_code ) . '" placeholder="' . esc_html__( 'Purchase code ( e.g. 9g2b13fa-10aa-2267-883a-9201a94cf9b5 )', 'wonderwall-magazine' ) . '"/>' . ' ' .
		__( 'Enter your purchase code and <strong>hit return/enter</strong>.', 'wonderwall-magazine' ) . ' ' .
		__( 'Find out how to <a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank">get your purchase code</a>.', 'wonderwall-magazine' ) .
		'</form>';
...
function wupdates_purchase_code_needed_notice() {
	...
	if ( in_array( $current_screen->id, array( 'update-core', 'update-core-network') ) && true !== wupdates_prevalidate_purchase_code( $purchase_code ) ) {
		$output .= '<div class="updated"><p>' . sprintf( __( '<a href="%s">Please enter your purchase code</a> to get automatic updates for <b>%s</b>.', 'wonderwall-magazine' ), network_admin_url( 'themes.php?page=wonderwall-magazine-getting-started' ), wp_get_theme( $slug ) ) . '</p></div>';
	}
...
function wupdates_process_purchase_code() {
	...
	if ( ! empty( $_POST['wupdates_pc_theme'] ) ) {
		...
		if ( ! empty( $purchase_code ) ) {
			...
			$url = $http_url = set_url_scheme( 'https://wupdates.com/wp-json/wup/v1/front/check_envato_purchase_code/r5ko9', 'http' );
			if ( $ssl = wp_http_supports( array( 'ssl' ) ) ) {
				$url = set_url_scheme( $url, 'https' );
			}
			...
			if ( is_wp_error( $raw_response ) || 200 != wp_remote_retrieve_response_code( $raw_response ) ) {
				$errors[] = __( 'We are sorry but we couldn\'t connect to the verification server. Please try again later.', 'wonderwall-magazine' ) . '<span class="hidden">' . print_r( $raw_response, true ) . '</span>';
			} elseif ( empty ( $errors ) ) {				
				...
				if ( ! empty( $response ) ) {
					...
					if ( isset( $response['purchase_code'] ) && 'valid' == $response['purchase_code'] ) {
						...
						// custom process
						wonderwall_magazine_process_activation(array(
						...
						if ( isset( $response['reason'] ) && ! empty( $response['reason'] ) && 'out_of_support' == $response['reason'] ) {
							$errors[] = esc_html__( 'Your purchase\'s support period has ended. Please extend it to receive automatic updates.', 'wonderwall-magazine' );
						} else {
							$errors[] = esc_html__( 'Could not find a sale with this purchase code. Please double check.', 'wonderwall-magazine' );
						}
					}
				}
			}
			...
			$errors[] = esc_html__( 'Please enter a valid purchase code. Make sure to get all the characters.', 'wonderwall-magazine' );
		}
